...
int send_folder(const char* folderpath, DIR *dirPtr, int fd_fifo, const char* named_fifo){
    extern struct cmd args;
    char buffer[args.buffer_sz], path[MAX_BUFFER];
    extern int fd_copy;
    ...
    while((direntPtr = readdir(dirPtr)) != NULL){
        ...
        write(fd_fifo, &bytes, sizeof(int));
...
int recv_folder(const char* dirname, int fd_fifo, const char* named_fifo){
    ...
    while((alarm_read(fd_fifo, &file_len, sizeof(uint8_t), MAX_WAIT_READ) == sizeof(uint8_t)) && (file_len > 0)){ //get length of file's name
        ...
        if(alarm_read(fd_fifo, &sz, sizeof(int), MAX_WAIT_READ) != sizeof(int))
